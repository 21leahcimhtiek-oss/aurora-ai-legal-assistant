  ...
  return (
    <View style={{ flex: 1, backgroundColor: DARK_BG }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 20, paddingTop: 56 }}>
        <View><Text style={{ fontSize: 22, fontWeight: "700", color: "#fff" }}>Hello, {user.name || "there"} ⚖️</Text><Text style={{ fontSize: 13, color: TEXT_SECONDARY }}>AI Assistant</Text></View>
      ...
      <ScrollView style={{ flex: 1, padding: 16 }} showsVerticalScrollIndicator={false}>
        ...
        {chatMessages.map(m => (<View key={m.id} style={{ padding: 14, borderRadius: 16, marginBottom: 10, maxWidth: "88%", alignSelf: m.role === "user" ? "flex-end" : "flex-start", backgroundColor: m.role === "user" ? PRIMARY_COLOR : CARD_BG, borderWidth: m.role === "assistant" ? 1 : 0, borderColor: BORDER_COLOR }}>{m.role === "assistant" && <Text style={{ fontSize: 11, color: TEXT_SECONDARY, marginBottom: 4 }}>⚖️ AI</Text>}<Text style={{ color: "#fff", fontSize: 15, lineHeight: 22 }}>{m.content}</Text></View>))}
        {isLoading && <View style={{ padding: 14, borderRadius: 16, marginBottom: 10, maxWidth: "60%", backgroundColor: CARD_BG, borderWidth: 1, borderColor: BORDER_COLOR }}><Text style={{ color: TEXT_SECONDARY }}>⚖️ Thinking...</Text></View>}
      </ScrollView>
